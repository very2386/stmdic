	...
	<div class="content clearfix">
		...
		<div class="home cat active all">
			<ul>
				...
				$two_news = \App\Cm::where($wh)->where('type','like','%'.$ntype.'%')->where('pic','!=','')->orderBy('psort','DESC')->orderBy('created_at','DESC')->limit(2)->get(); 
		...
		if(request('sort')&&request('sort')=='hot') $query->orderby('hits','desc');
			...
			$news = $query->where('pic','!=','')->orderby('created_at','DESC')->paginate(6);
			$no_pic = \App\Cm::where($wh)->where('type','like','%'.$type.'%')->where('pic','')->orderby('created_at','DESC')->paginate(18);
			if(count($no_pic)!=32){
				$other_news = \App\Cm::where($wh)->where('type','like','%'.$type.'%')->orderby('created_at','DESC')->paginate(18-count($no_pic));
			}
		}
		$hot_news = \App\Cm::where($wh)->where('type','like','%'.$type.'%')->orderby('hits','desc')->orderBy('pic','DESC')->limit(2)->get();
		?>
			...
			<div class="content two-side news-wrap news-inner">
				<div class="side-left">
					<ul class="grid2 spacing15 no-padding wrap">
						@foreach($news as $n)
						<li>
							<a href="/news_content/{{$n->id}}" title="{{$n->name}}">
								<span class="img"><img src="{{\App\Cm::get_news_pic($n->id)}}"></span>	
								<span class="text">{{$n->name}}</span>
							</a>
							<p class="source"><span class="date">{{$n->created_at}}</span> <span class="from">{{$n->up_sn!=0?\App\Cm::get_sourcename($n->id):''}}</span></p>
						</li>
						@endforeach
					</ul>
				</div>
						...
						<li class="ca">
							<div class="text">
								<ul>
									...
									<li style="display:flex;align-items:center;height: 50px;">
										<a href="/news_content/{{$rd->id}}" style="white-space: normal;height: 2.5em;overflow: hidden;display: -webkit-box;-webkit-line-clamp: 2;-webkit-box-orient: vertical;" title="{{$rd->name}}">{{$rd->name}}
										</a>
									</li>
									@endforeach
									@if(count($no_pic)!=32)
										@foreach($other_news as $rd)
										<li style="display:flex;align-items:center;height: 50px;">
											<a href="/news_content/{{$rd->id}}" style="white-space: normal;height: 2.5em;overflow: hidden;display: -webkit-box;-webkit-line-clamp: 2;-webkit-box-orient: vertical;" title="{{$rd->name}}">{{$rd->name}}
											</a>
										</li>
						...
						<!-- <li class="hot-post">
							...
							<div class="posts">
								<ul>
									...
									<li>
										<a href="/news_content/{{$rd->id}}" title="{{$rd->name}}">
