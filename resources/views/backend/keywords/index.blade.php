		...
		<div class="card card-banner no-br">
			<div class="card-header">
			  <div class="card-title">
			    <div class="title">關鍵字管理</div>
			  </div>
			  <ul class="card-action">
			    <li><a class="btn btn-primary" href="/backend/keywords/keywords_edit">新增</a></li>
			  </ul>
			</div>
			<div class="card-body">
				<?php
				...
				<table class="table datatable">
					...
					<tbody>
						<?php
						$rs = \App\Cm::where('position', 'tag')->where('type','text')->where('lang', session('lang'))->orderBy('psort', 'desc')->orderBy('id', 'desc')->get();
						foreach($rs as $rd):
						...
						?>
							<tr id="item_row{{$rd->id}}">
								...
								<td>
									...
									<?php $tagrd = \App\Tags::where('type', 'news')->where('name', $tagname)->first();?>
									<span class="tag" style="background:{{$tagrd->color}}">{{$tagrd->name}}</span>
									@endforeach
								</td>
