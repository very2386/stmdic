		...
		<div class="card card-banner no-br">
			<div class="card-header">
			  <div class="card-title">
			    <div class="title">GUNPLA商品</div>
			  </div>
			  <ul class="card-action">
			    <li><a class="btn btn-primary" href="/backend/gunpla/prod_edit">新增</a></li>
			  </ul>
			</div>
			<form name="filter_form" action="/backend/gunpla/index" method="get">
				<div class="form-inline filter">
					系列：
					<?php
					$selrs = \App\Cm::where('position', 'series')->where('type', 'gunpla')->where('mstatus','Y')->orderBy('psort', 'desc')->get();
					?>
					...
					作品：
					<?php
					$selrs = \App\Cm::where('position', 'works')->where('type', 'gunpla')->where('mstatus','Y')->orderBy('psort', 'desc')->get();
					?>
					...
					<tbody>
						...
						?>
							<tr id="item_row{{$prd->id}}">
								...
								<th>
									<a class="btn btn-primary" href="/backend/gunpla/prod_edit?id={{$rd->id}}">編輯</a>
									<a class="btn btn-danger" onclick="del_item( 'products', '{{$prd->id}}');" >刪除</a>
								</th>
							</tr>
						<?php 
						endforeach;
